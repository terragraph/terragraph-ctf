...
def parse_args():
    parser = argparse.ArgumentParser(description="Verify scan results.")
    ...
    parser.add_argument("--ignore", help="List of nodes to ignore", type=str)
    return parser.parse_args()


def bwgdToTs(bwgd):
    realGpsTime = bwgd * 256 / 10
...
def get_relative_beams(beam):
    if beam == -1:
        ...
        # Update AWV PBF uses scan range beam indexing
        beam_list = list(range(64, 32))
        beam_list = range(128)
    else:
...
def verif_status(res, scan, tag, node):
    ...
    if node == "" or node not in scan["responses"].keys():
        subres = "FAIL"
        status = "?"
    else:
        subres = "PASS"
...
def verif_rxstart(res, response):
    ...
    routes = [] if "routeInfoList" not in response.keys() else response["routeInfoList"]
    if "startSuperframeNum" not in response.keys() or len(routes) == 0:
        return res
...
def average_routes(scanType, subType, routes, tx_list, rx_list, mode="DEFAULT"):
    res = "PASS"

    if scanType == "CBF_TX" or scanType == "CBF_RX":
        num_beams = NUM_BEAMS_CBF
    elif scanType == "IM" and mode == "RELATIVE":
        num_beams = NUM_BEAMS_EIM
    else:
        num_beams = thresh.num_beams
    ...
    # if scanType == 'PBF':
    #    beam_range = range(63,31,-1) + range(0,32)
    # else:
    #    beam_range = range(0, num_beams)
    beam_range = range(0, num_beams)
    # Print results
...
def verif_routes(scanType, subType, routes, expected_num_routes, mode="DEFAULT"):
    ...
    beam_cnt_snr = np.zeros(shape=(num_beams, num_beams, 2))
    min_snr = 999
    low_snr_cnt = 0
    for r in routes:
        pkt = r["packetIdx"]
        ...
        if r["snrEst"] > MIN_SNR_DOUBLE_PACKET_CHECK:
            beam_cnt_snr[txidx][rxidx][pkt] = beam_cnt_snr[txidx][rxidx][pkt] + 1
    ...
    for pkt in range(0, 2):
        for txb in range(0, num_beams):
            for rxb in range(0, num_beams):
                pkt_cnt[pkt] = pkt_cnt[pkt] + int(beam_cnt_snr[txb][rxb][pkt])
    ...
    if (scanType == "CBF_TX" or scanType == "CBF_RX") and (
        ...
        res = log_subtest(
            res,
            "Double packet match to {:d} packets or {:d}% for packets with SNR > {:d} dB ({:d}, {:d})".format(
                thresh.double_pkt_diff,
                thresh.double_pkt_rel,
                MIN_SNR_DOUBLE_PACKET_CHECK,
                pkt_cnt[0],
                pkt_cnt[1],
            ),
            subres,
...
def verif_pbf_rtcal(scan):
    ...
    subType = "NO_CAL" if "subType" not in scan.keys() else SubTypes[scan["subType"]]
    ...
    txb = (
        -1
        if tx == "" or "oldBeam" not in scan["responses"][tx].keys()
        else scan["responses"][tx]["oldBeam"]
    )
    rxb = (
        -1
        if rx == "" or "oldBeam" not in scan["responses"][rx].keys()
    ...
    apply = scan["apply"] if "apply" in scan.keys() else False
    new_tx = (
        -1
        if tx == "" or "newBeam" not in scan["responses"][tx].keys()
        else scan["responses"][tx]["newBeam"]
    )
    new_rx = (
        -1
        if rx == "" or "newBeam" not in scan["responses"][rx].keys()
    ...
    if tx_status != "SCAN_PROCEDURE_COMPLETE" or rx_status != "SCAN_PROCEDURE_COMPLETE":
        ...
        if (
            subType == "NO_CAL"
            and new_tx in range(0, thresh.num_beams)
            and new_rx in range(0, thresh.num_beams)
        ):
            subres = "PASS"
        elif (
...
def verif_cbf(scan):
    ...
    if "mainTxNode" in scan.keys():
        ...
        for node in list(scan["responses"]):
            if node == scan["txNode"]:
                main_tx = node
            elif "txPwrIndex" in scan["responses"][node].keys():
                aux_tx = node
            else:
    ...
    routes = (
        scan["responses"][main_rx]["routeInfoList"]
        if main_rx in scan["responses"]
        else []
    )
    ...
    routes = (
        scan["responses"][aux_rx]["routeInfoList"]
        if aux_rx in scan["responses"]
        else []
    )
...
def verif_im(scan):
    ...
    tx = scan["txNode"]
    if tx in ignore_nodes:
        return "IGNORE"
    if tx in scan["responses"].keys():
    ...
    for node in list(scan["responses"]):
        ...
        if len(routes) == 0:
            ...
            subres, beam_cnts = verif_routes(
                "IM", "NO_CAL", routes, thresh.im_route_cnt, mode
            )
            if subres == "PASS":
                num_resp_exp_routes = num_resp_exp_routes + 1
            # res = verif_rxstart(res, scan['responses'][node])
        subres = average_routes("IM", "NO_CAL", routes, [], [], mode)
...
def main():
    ...
    if args.rf:
        print("Using thresholds for QCOM RF setup")
        thresh = Threshold(
            pbf_fine_route_cnt=50,
            pbf_rel_route_cnt=20,
            im_route_cnt=0,
            rtcal_route_cnt=50,
            vbs_route_cnt=10,
            cbf_route_cnt=100,
        ...
        print("Using thresholds for QCOM IF setup (add --rf for RF thresholds)")
        thresh = Threshold(
            pbf_fine_route_cnt=4096,
            pbf_rel_route_cnt=25,
            im_route_cnt=4096,
    ...
    if args.routes:
        print("Showing averaged route info")
        show_routes = True
    else:
        show_routes = False

    if args.ignore:
        ignore_nodes = args.ignore
        print("Ignoring nodes: %s" % ignore_nodes)
    else:
        ignore_nodes = ""
    ...
    for token in tokens:
        ...
        try:
            ...
            scanType = ScanTypes[scan["type"]]
            subType = (
                "NO_CAL" if "subType" not in scan.keys() else SubTypes[scan["subType"]]
            )
            mode = ScanMode[scan["mode"]]
            test_str = "Token %s: %s, %s, %s, TX:%s @ %f (%s), %d responses" % (
                token,
                scanType,
                subType,
                mode,
            ...
            res = log_subtest(res, "Scan slot: {:f}".format(scanSlot), "IGNORE")
