...
class TestPumaE2EGolayDiffResp(TestX86TGIgn):
    TEST_NAME = "Puma RF golay"
    DESCRIPTION = "Update link golays with different responder and initiator golays using apis after link is ignited by e2e controller."
    ...
    def get_test_steps(self) -> List[Dict]:
        ...
        for golay_idx in golay_test_idxs:
            responder_golay_idx = golay_idx - 2
            config_dict: Dict = {}
            config_dict["key"] = self.test_options["override_responder_key_tx"]
            config_dict["value"] = responder_golay_idx
            test_overrides = self.create_node_overrides(
                config_dict,
                node_names=[self.test_options["responder_node_name"]],
            )
            steps.append(
                {
                    "name": f"e2e rest API to Modify Node Overrides on responder with txGolayIdx#{responder_golay_idx}",
                    "function": self.api_service_request,
                    "function_args": (
            ...
            config_dict: Dict = {}
            config_dict["key"] = self.test_options["override_responder_key_rx"]
            config_dict["value"] = responder_golay_idx
            test_overrides = self.create_node_overrides(
                config_dict,
                node_names=[self.test_options["responder_node_name"]],
            ...
            steps.append(
                {
                    "name": f"e2e rest API to Modify Node Overrides on responder with rxGolayIdx#{responder_golay_idx}",
                    "function": self.api_service_request,
                    "function_args": (
                        "modifyNodeOverridesConfig",
                        test_overrides,
