...
class TestPumaE2EGolayDiffRx(TestX86TGIgn):
    TEST_NAME = "Puma RF golay"
    DESCRIPTION = "Update link golays with different tx and rx golays using apis after link is ignited by e2e controller."
    ...
    def get_test_steps(self) -> List[Dict]:
        ...
        for golay_idx in golay_test_idxs:
            tx_golay_idx = golay_idx
            rx_golay_idx = golay_idx - 2
            config_dict: Dict = {}
            config_dict["key"] = self.test_options["override_key_tx"]
            config_dict["value"] = tx_golay_idx
            ...
            steps.append(
                {
                    "name": f"e2e rest API to Modify Node Overrides on initiator with txGolayIdx#{tx_golay_idx}",
                    "function": self.api_service_request,
                    "function_args": (
            ...
            config_dict["value"] = rx_golay_idx
            test_overrides = self.create_node_overrides(
                config_dict,
                node_names=[self.test_options["initiator_node_name"]],
            )
            steps.append(
                {
                    "name": f"e2e rest API to Modify Node Overrides on initiator with rxGolayIdx#{rx_golay_idx}",
                    "function": self.api_service_request,
                    "function_args": (
                        "modifyNodeOverridesConfig",
                        test_overrides,
